...
load(here("week-2", "sierraczosolar.rda"))
...
-   what issue jumps out - what might be the cause?
    -   1954 is lower because it doesn't have a complete year!
-   how would you fix this?
    -   Exclude the year if you don't need it
...
site1 <- solarpv(area = 0.1, 
          solar = sierraczosolar, clr = "forestgreen", eunit = "W", g = TRUE)
...
site2 <- eff %>% map(~ solarpv(area = 0.1, 
  solar = sierraczosolar, clr = "green", 
  eunit = "W", g = FALSE, etype = "direct", eff = .x)) # .x in the thing that im inputting
...
tmp <- map_dfr(site2, `[`, c("annual")) # extract something and put it into a dataframe by row

site2df <- data.frame(year = tmp$annual$year, elect = tmp$annual$elect)
head(site2df) # this has values for all of the efficiencies
```
